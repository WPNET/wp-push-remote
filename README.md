...
A powerful bash script to push WordPress sites from a SOURCE server to a REMOTE server using WP-CLI and rsync. Features include database migration with search-replace, file synchronization with exclusions, and an interactive configuration mode.
...
- 🔐 **SSH Key Management**: Automatic SSH key generation and setup
...
- **v2.0.0**: Major update with interactive configuration, colorized output, comprehensive argument parsing, and improved UX
